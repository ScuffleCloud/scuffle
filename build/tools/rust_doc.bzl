...
load("@rules_rust//rust/private:utils.bzl", "dedent", "find_cc_toolchain", "find_toolchain", "transform_deps")
...
def _zip_action(ctx, input_dir, output_zip):
    ...
    ctx.actions.run(
        ...
        arguments = [args],
        mnemonic = "RustdocZip",
        progress_message = "Creating RustdocZip",
...
def _sort_topological_rev(crates):
    ...
    We want the reverse because b needs c and a needs b so we need to build
    c -> b -> a
    """
    ...
    for crate in crates:
        dep_info, _, _ = collect_deps(
            deps = depset(transform_deps([crate])),
...
def _rust_doc_impl(ctx):
    ...
    for crate in _sort_topological_rev(ctx.attr.crates):
        env_file = ctx.actions.declare_file("{}.{}.env.txt".format(ctx.label.name, str(crate.label).replace("/", ".")))
        ...
        for feature in crate_info.crate_features:
            rustdoc_flags.append("--cfg=feature=\"{}\"".format(feature))
        ...
        if ctx.attr.env:
            action.env.update(ctx.attr.env)

        all_inputs.extend(action.inputs.to_list())
        all_inputs.extend([env_file, args_file])
        all_tools.extend(action.tools)
        all_tools.append(action.executable)

        ctx.actions.write(
            env_file,
            content = "\n".join(["{}={}".format(env, var) for env, var in action.env.items()]),
        )
        ...
        script_lines.append("{} --env-file {} --arg-file {} -- {}".format(
