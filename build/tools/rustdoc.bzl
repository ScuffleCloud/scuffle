...
def _init_rust_doc_info(*, html_out = None, json_out = None, parts_out = None):
...
def _rustdoc_impl(ctx):
    ...
    if ctx.attr.generate_parts:
        if ctx.attr.output_format != "html":
            fail("Generate parts can only be used with output_format = \"html\"")

        parts_out = ctx.actions.declare_directory("{}.rustdoc.parts".format(ctx.label.name))
        rustdoc_flags.extend(["--parts-out-dir", parts_out.path])
        outputs.append(parts_out)

    if ctx.attr.output_format == "html":
        html_out = ctx.actions.declare_directory("{}.rustdoc".format(ctx.label.name))
        rustdoc_flags.extend(["--out-dir", html_out.path])
...
rustdoc = rule(
    ...
    attrs = {
        ...
        "_rustdoc_wrapper": attr.label(
            default = Label("//build/tools/rustdoc_wrapper"),
            cfg = "exec",
            executable = True,
        ),
...
rustdoc_test = rule(
    ...
    attrs = {
        ...
        "_rust_doctest_builder": attr.label(
            default = Label("//build/tools/rust_doctest_builder"),
            cfg = "exec",
            executable = True,
        ),
        "_rust_doctest_runner": attr.label(
            default = Label("//build/tools/rust_doctest_runner"),
            cfg = "exec",
            executable = True,
        ),
        "_process_wrapper": attr.label(
