...
impl Extensions {
    ...
    pub fn messages(&self) -> impl Iterator<Item = (&String, &MessageOpts)> {
        self.messages.iter()
    }

    pub fn enums(&self) -> impl Iterator<Item = (&String, &EnumOpts)> {
        self.enums.iter()
    }

    pub fn services(&self) -> impl Iterator<Item = (&String, &ServiceOpts)> {
        self.services.iter()
    }
