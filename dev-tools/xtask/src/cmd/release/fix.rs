    ...
    pub fn run(mut self) -> anyhow::Result<()> {
        ...
        for package in groups.values().flatten() {
            ...
            for dep in &package.dependencies {
                ...
                let version = if dep_pkg.group() == package.group() {
                    semver::VersionReq {
                ...
                changelogs.push(
                    PackageChangeLog::new("chore", format!("bumped `{}` to `{version}`", dep.name))
                        .with_version_bump(&dep.name, next_dep_version),
                );
                table[&dep.name]["version"] = version.into();
            }
